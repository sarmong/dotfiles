...
expand_properties = function(text, magic)
  ...
  for prefix, raw, prop, colon, fallback, closing in
    text:gmatch("%" .. magic .. "{([?!]?)(=?)([^}:]*)(:?)([^}]*)(}*)}")
  do
    local err
    local prop_value
...
format_filename = function(startTime, endTime, videoFormat)
  ...
  local replaceTable = {
    ...
    ["%%M"] = (mp.get_property_native("aid") and not mp.get_property_native(
      "mute"
    ) and hasAudioCodec) and "-audio" or "",
    ["%%R"] = (options.scale_height ~= -1) and "-" .. tostring(
      options.scale_height
    ) .. "p" or "-" .. tostring(mp.get_property_native("height")) .. "p",
    ["%%t%%"] = "%%",
  }
...
get_video_dimensions = function()
  ...
  _video_dimensions.ratios.w = w
    / (_video_dimensions.bottom_right.x - _video_dimensions.top_left.x)
  _video_dimensions.ratios.h = h
    / (_video_dimensions.bottom_right.y - _video_dimensions.top_left.y)
  return _video_dimensions
end
...
do
  ...
  local _base_0 = {
    ...
    set_duration = function(self, duration)
      local big_endian_binary_duration =
        write_double(duration * duration_multiplier)
    ...
    as_binary_string = function(self)
      local before_duration_string =
        self:_bytes_to_string(self.binary_data_before_duration)
      local duration_string = self:_bytes_to_string(self.binary_duration)
      local after_duration_string =
        self:_bytes_to_string(self.binary_data_after_duration)
  ...
  local _base_0 = {
    getPreFilters = function(self)
      ...
      if colormatrixFilter[colormatrix] then
        append(ret, {
          "lavfi-colormatrix="
            .. tostring(colormatrixFilter[colormatrix])
            .. ":bt601",
        })
      end
      return ret
  ...
  local _base_0 = {
    postCommandModifier = function(self, command, region, startTime, endTime)
      ...
      for _, v in ipairs(command) do
        ...
        repeat
          if
            v:match("^%-%-vf%-add=lavfi%-scale")
            or v:match("^%-%-vf%-add=lavfi%-crop")
            or v:match("^%-%-vf%-add=fps")
            or v:match("^%-%-vf%-add=lavfi%-eq")
          then
      ...
      if options.gif_dither == 6 then
        ...
        cfilter = cfilter
          .. "[vidf][pal]paletteuse=dither=bayer:bayer_scale="
          .. tostring(options.gif_dither)
          .. ":diff_mode=rectangle[vo]"
      end
      append(new_command, {
...
do
  ...
  local _base_0 = {
    draw = function(self)
      local progress = 100
        * ((self.currentTime - self.startTime) / self.duration)
...
append_track = function(out, track)
  ...
  if track["external"] and string.len(track["external-filename"]) <= 2048 then
    return append(out, {
      "--" .. tostring(external_flag[track["type"]]) .. "=" .. tostring(
        track["external-filename"]
      ),
    })
  else
    return append(out, {
      "--" .. tostring(internal_flag[track["type"]]) .. "=" .. tostring(
        track["id"]
      ),
    })
  end
...
get_contrast_brightness_and_saturation_filters = function()
  ...
  return {
    "lavfi-eq=contrast="
      ...
      .. ":saturation="
      .. tostring(eq_saturation)
      .. ":brightness="
      .. tostring(eq_brightness),
  }
end
...
apply_current_filters = function(filters)
  ...
  msg.verbose(
    "apply_current_filters: got " .. tostring(#vf) .. " currently applied."
  )
  for _index_0 = 1, #vf do
    ...
    repeat
      ...
      msg.verbose(
        "apply_current_filters: filter name: " .. tostring(filter["name"])
      )
...
calculate_bitrate = function(active_tracks, format, length)
  ...
  local audio_bitrate = audio_kilobits and math.floor(audio_kilobits / length)
    or nil
...
encode = function(region, startTime, endTime)
  ...
  path, is_stream, is_temporary, startTime, endTime =
  ...
  local formatted_filename =
    format_filename(originalStartTime, originalEndTime, format)
  ...
  if options.twopass and format.supportsTwopass and not is_stream then
    ...
    msg.verbose(
      "First-pass command line: ",
      table.concat(first_pass_cmdline, " ")
    )
  ...
  local _base_0 = {
    ...
    draw = function(self)
      ...
      ass:append(
        tostring(bold("1:"))
          .. " change point A ("
          .. tostring(self.pointA.x)
          .. ", "
          .. tostring(self.pointA.y)
          .. ")\\N"
      )
      ass:append(
        tostring(bold("2:"))
          .. " change point B ("
          .. tostring(self.pointB.x)
          .. ", "
          .. tostring(self.pointB.y)
          .. ")\\N"
      )
...
do
  ...
  local _base_0 = {
    ...
    getDisplayValue = function(self)
      ...
      elseif "int" == _exp_0 then
        if
          self.opts.altDisplayNames and self.opts.altDisplayNames[self.value]
        then
    ...
    draw = function(self)
      ...
      ass:append(
        tostring(bold("1:"))
          ...
          .. tostring(seconds_to_time_string(self.startTime))
          .. ")\\N"
      )
      ass:append(
        tostring(bold("2:"))
          .. " set end time (current is "
          .. tostring(seconds_to_time_string(self.endTime))
          .. ")\\N"
      )
