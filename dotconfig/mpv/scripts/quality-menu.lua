...
opts.font_size = tonumber(opts.style_ass_tags:match("\\fs(%d+%.?%d*)"))
  or mp.get_property_number("osd-font-size")
  or 25
...
local function process_json(json)
  ...
  local sort_video, reverse_video =
    strip_minus(string_split(opts.sort_video, ","))
  local sort_audio, reverse_audio =
    strip_minus(string_split(opts.sort_audio, ","))
  ...
  local function format_special_fields(format)
    local size_prefix = not format.filesize and format.filesize_approx and "~"
      or ""
    ...
    format.codec_video = format.vcodec == nil and "unknown"
      or format.vcodec == "none" and ""
      or format.vcodec
    format.codec_audio = format.acodec == nil and "unknown"
      or format.acodec == "none" and ""
      or format.acodec
  ...
  local function format_table(formats, columns)
    ...
    for _, f in ipairs(formats) do
      ...
      for i, column in ipairs(show_columns) do
        ...
        local width =
          math.min(column.width * (column.align_left and -1 or 1), 99)
        row = row
...
local function download_formats(url)
  ...
  local function check_version(ytdl_path)
    ...
    if os.difftime(os.time(), version_ts) > 60 * 60 * 24 * 90 then
      msg.warn(
        "It appears that your youtube-dl version is severely out of date."
      )
    end
  end
...
local function show_menu(isvideo)
  ...
  local function get_scrolled_lines()
    local output_height = height
      - opts.text_padding_y * 2
      - margin_top * height
    ...
    return math.min(
      math.max(selected - math.ceil(screen_lines / 2), 0),
      max_scroll
    )
  end
  ...
  local function draw_menu()
    ...
    ass.text = string.format(
      "{\\pos(0,0)\\rDefault\\an7\\1c&H000000&\\alpha&H%X&}",
      alpha
    )
    ass:draw_start()
    ...
    local pos_y = opts.shift_y
      + margin_top * height
      + opts.text_padding_y
      - scrolled_lines * opts.font_size
    ...
    local clipping_coordinates = "0,"
      ...
      .. clip_bottom
    ass:append(
      opts.style_ass_tags .. "{\\q2\\clip(" .. clipping_coordinates .. ")}"
    )
