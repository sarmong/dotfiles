...
function update_opts(changelog)
  ...
  if changelog.playlist_display_timeout then
    keybindstimer =
      mp.add_periodic_timer(settings.playlist_display_timeout, remove_keybinds)
    keybindstimer:kill()
  end
...
function on_loaded()
  ...
  if
    path:match("^https?://")
    and not url_table[path]
    and path ~= media_title
  then
  ...
  if settings.set_title_stripped then
    mp.set_property(
      "title",
      settings.title_prefix .. strippedname .. settings.title_suffix
    )
  end
...
function stripfilename(pathfile, media_title)
  ...
  if settings.filename_replace and not media_title then
    for k, v in ipairs(settings.filename_replace) do
      if
        (
          v["ext"]
          and (v["ext"][ext] or (ext and not protocol and v["ext"]["all"]))
        )
        or (
          v["protocol"]
          and (
            v["protocol"][protocol]
            or (protocol and not ext and v["protocol"]["all"])
          )
        )
      then
  ...
  if
    settings.slice_longfilenames
    and tmp:len() > settings.slice_longfilenames_amount + 5
  then
...
function parse_header(string)
  local esc_title =
    stripfilename(mp.get_property("media-title"), true):gsub("%%", "%%%%")
...
function draw_playlist()
  ...
  if
    mp.get_property("osd-align-x") == "left"
  ...
  for index = start, start + settings.showamount - 1, 1 do
    ...
    if
      index == start + settings.showamount - 1
      and not showall
      and not showrest
    then
  ...
  if files then
    ...
    for _, file in ipairs(files) do
      ...
      elseif cur == true or settings.loadfiles_always_append then
        ...
        mp.commandv(
          "playlist-move",
          mp.get_property_number("playlist-count", 1) - 1,
          c
        )
        c = c + 1
      end
...
function activate_playlist_save()
  if interactive_save then
    msg.info("saving interactively..........")
    remove_keybinds()
    mp.command(
      'script-message playlistmanager-save-interactive "start interactive filenaming process"'
    )
  else
    msg.info("saving non interactively..........")
    save_playlist()
  end
end
...
function save_playlist(filename)
  ...
  if utils.readdir(savepath) == nil then
    ...
    if res.status ~= 0 then
      msg.error(
        "Failed to create playlist save directory "
          .. savepath
          .. ". Error: "
...
function add_keybinds()
  ...
  bind_keys_forced(
    settings.key_movepageup,
    "movepageup",
    movepageup,
    "repeatable"
  )
  bind_keys_forced(
    settings.key_movepagedown,
    "movepagedown",
    movepagedown,
    "repeatable"
  )
...
if
  settings.loadfiles_on_idle_start
  and mp.get_property_number("playlist-count", 0) == 0
then
  playlist()
end
    ...
    then
      ...
      local req = mp.command_native_async({
        ...
        if res.killed_by_us then
          msg.verbose(
            "Request to resolve url title " .. filename .. " timed out"
          )
          return
        end
        if res.status == 0 then
          ...
          if not err then
            ...
            local title = (is_playlist and "[playlist]: " or "")
              .. json["title"]
...
function handlemessage(msg, value, value2)
  ...
  if msg == "save" then
    if value == "interactively" then
      activate_playlist_save()
    else
      save_playlist(value)
    end
    return
