...
o.history_incognito_mode_keybind =
  utils.parse_json(o.history_incognito_mode_keybind)
...
o.header_remaining_time_format =
  utils.parse_json(o.header_remaining_time_format)
...
o.list_unhighlight_all_keybind =
  utils.parse_json(o.list_unhighlight_all_keybind)
...
o.list_add_playlist_highlighted_keybind =
  utils.parse_json(o.list_add_playlist_highlighted_keybind)
o.list_close_keybind = utils.parse_json(o.list_close_keybind)
o.list_delete_keybind = utils.parse_json(o.list_delete_keybind)
o.list_delete_highlighted_keybind =
  utils.parse_json(o.list_delete_highlighted_keybind)
o.list_search_activate_keybind =
  utils.parse_json(o.list_search_activate_keybind)
o.list_search_not_typing_mode_keybind =
  utils.parse_json(o.list_search_not_typing_mode_keybind)
o.next_filter_sequence_keybind =
  utils.parse_json(o.next_filter_sequence_keybind)
o.previous_filter_sequence_keybind =
  utils.parse_json(o.previous_filter_sequence_keybind)
...
function format_time(seconds, sep, decimals, style)
  ...
  if not style or style == "" or style == "default" then
    local second_format = string.format(
      "%%0%d.%df",
      2 + (decimals > 0 and decimals + 1 or 0),
      decimals
    )
    sep = sep and sep or ":"
    return string.format(
      "%02d" .. sep .. "%02d" .. sep .. second_format,
      h,
      m,
      s
    )
  elseif style == "hms" or style == "hms-full" then
    ...
    if style == "hms-full" or h > 0 then
      return string.format(
        "%dh" .. sep .. "%dm" .. sep .. "%." .. tostring(decimals) .. "fs",
        h,
        m,
        s
      )
...
function get_file()
  ...
  if
    starts_protocol(o.logging_protocols, path)
    and o.prefer_filename_over_title == "protocols"
  then
    title = mp.get_property("filename"):gsub('"', "")
  elseif
    not starts_protocol(o.logging_protocols, path)
    and o.prefer_filename_over_title == "local"
  then
    title = mp.get_property("filename"):gsub('"', "")
...
function read_log_table()
  ...
  return read_log(function(line)
    ...
    if line:match('^.-"(.-)"') then
      tt = line:match('^.-"(.-)"')
      n, p = line:match(
        '^.-"(.-)" | (.*) | ' .. esc_string(log_length_text) .. "(.*)"
      )
    else
      p =
        line:match("[(.*)%]]%s(.*) | " .. esc_string(log_length_text) .. "(.*)")
      d, n, e = p:match("^(.-)([^\\/]-)%.([^\\/%.]-)%.?$")
    end
    dt = line:match("%[(.-)%]")
...
function list_sort(tab, sort)
  ...
  elseif sort == "alphanum-asc" or sort == "alphanum-desc" then
    ...
    if sort == "alphanum-asc" then
      table.sort(tab, function(a, b)
        return tostring(a["found_path"]):gsub("%.?%d+", padnum)
            .. ("%3d"):format(#b)
          > tostring(b["found_path"]):gsub("%.?%d+", padnum)
            .. ("%3d"):format(#a)
      end)
...
function parse_header(string)
  ...
  string =
    string:gsub("%%total%%", #list_contents):gsub("%%cursor%%", list_cursor)
  ...
  if list_total_duration > 0 then
    ...
    string =
      string:gsub("%%prelistduration%%", ""):gsub("%%afterlistduration%%", "")
  end
  ...
  if list_total_length > 0 then
    ...
    string =
      string:gsub("%%prelistlength%%", ""):gsub("%%afterlistlength%%", "")
  end
  ...
  if list_total_remaining > 0 then
    ...
    string =
      string:gsub("%%prelistremaining%%", ""):gsub("%%afterlistremaining%%", "")
  end
  ...
  if search_active then
    ...
    if search_string_osd ~= "" then
      search_string_osd = search_string
        :gsub("%%", "%%%%%%%%")
        :gsub("\\", "\\​")
        :gsub("{", "\\{")
    end
    ...
    string = string
      :gsub(
        "%%search%%",
        osd_search_color .. search_string_osd .. osd_header_color
      )
...
function get_list_contents(filter, sort)
  ...
  if
    not list_contents and not search_active
    or not list_contents[1] and not search_active
  then
    return
  end
  ...
  if search_active and search_string ~= "" then
    ...
    for i = 1, #list_contents do
      if o.search_behavior == "specific" then
        if
          string
            .lower(list_contents[i].found_path)
            :match(string.lower(search_query))
        then
          table.insert(filtered_table, list_contents[i])
        elseif
          list_contents[i].found_title
          and string
            .lower(list_contents[i].found_title)
            :match(string.lower(search_query))
        then
          table.insert(filtered_table, list_contents[i])
        elseif
        ...
        elseif
          string
            .lower(list_contents[i].found_datetime)
            :match(string.lower(search_query))
        then
          table.insert(filtered_table, list_contents[i])
        end
...
function draw_list()
  ...
  for i = list_start, list_start + o.list_show_amount - 1, 1 do
    ...
    if o.show_paths then
      p = list_contents[#list_contents - i].found_path
        or list_contents[#list_contents - i].found_name
        or ""
    else
      p = list_contents[#list_contents - i].found_name
        or list_contents[#list_contents - i].found_path
        or ""
    end
    ...
    if
      o.quickselect_0to9_keybind
      and o.list_show_amount <= 10
      and o.quickselect_0to9_pre_text
    then
      key = 1 + key
      if key == 10 then
        key = 0
    ...
    if
      i == list_start + o.list_show_amount - 1
      and not showall
      and not showrest
    then
...
function display_list(filter, sort, action)
  ...
  for i = 1, #list_pages do
    ...
    if
      list_pages[i][3] == 2
      and filter == "all"
      and o.main_list_keybind_twice_exits
    then
  ...
  if list_cursor_temp > 0 and list_cursor_temp <= #list_contents then
    ...
    if action == "highlight" then
      if not has_value(list_highlight_cursor, list_cursor, 1) then
          ...
          for i = pos, 1, -1 do
            if not has_value(list_highlight_cursor, list_cursor - i, 1) then
              table.insert(
                ...
                {
                  list_cursor - i,
                  list_contents[#list_contents + 1 + i - list_cursor],
                }
              )
            end
          end
        ...
        table.insert(
          list_highlight_cursor,
          { list_cursor, list_contents[#list_contents + 1 - list_cursor] }
        )
      else
        for i = 1, #list_highlight_cursor do
          if
            list_highlight_cursor[i]
            and list_highlight_cursor[i][1] == list_cursor
          then
        ...
        if pos > -1 then
          for i = 1, #list_highlight_cursor do
            for j = pos, 1, -1 do
              if
                list_highlight_cursor[i]
                and list_highlight_cursor[i][1] == list_cursor - j
              then
                table.remove(list_highlight_cursor, i)
              end
            end
  ...
  if #list_highlight_cursor < #list_contents then
    for i = 1, #list_contents do
      if not has_value(list_highlight_cursor, i, 1) then
        table.insert(
          list_highlight_cursor,
          { i, list_contents[#list_contents + 1 - i] }
        )
      end
    end
    select(0)
...
function load(list_cursor, add_playlist, target_time)
  ...
  if not target_time then
    seekTime = tonumber(
      list_contents[#list_contents - list_cursor + 1].found_time
    ) + o.resume_offset
    if seekTime < 0 then
      seekTime = 0
    end
  ...
  elseif action == "highlight" then
    ...
    if file_ignored_total > 0 then
      msg.warn(
        "Ignored a total of "
          .. file_ignored_total
          .. " Item/s that does not seem to exist"
      )
    end
    msg.info(
      "Added into playlist a total of "
        .. #list_highlight_cursor - file_ignored_total
        .. " item/s"
    )
  end
...
function delete_selected()
  ...
  seekTime =
    tonumber(list_contents[#list_contents - list_cursor + 1].found_time)
...
function get_total_duration(action)
  ...
  if
    action == "found_time"
    or action == "found_length"
    or action == "found_remaining"
  then
    for i = #list_contents, 1, -1 do
...
function update_list_highlist_cursor()
  ...
  for i = 1, #list_contents do
    for j = 1, #list_highlight_cursor do
      if
        list_contents[#list_contents + 1 - i].found_sequence
        == list_highlight_cursor[j][2].found_sequence
      then
        table.insert(
          temp_list_highlight_cursor,
          { i, list_highlight_cursor[j][2] }
        )
      end
    end
  end
...
function select_filter_sequence(pos)
  ...
  if o.loop_through_filters then
    if
      not target_pos and pos > -1
      or target_pos and target_pos > #o.filters_and_sequence
    then
      for i = 1, #o.filters_and_sequence do
...
function get_list_keybinds()
  ...
  bind_keys(
    o.previous_filter_sequence_keybind,
    "list-filter-previous",
    list_filter_previous
  )
  bind_keys(
    o.list_search_activate_keybind,
    "list-search-activate",
    list_search_activate
  )
  bind_keys(
    o.list_highlight_all_keybind,
    "list-highlight-all",
    list_highlight_all
  )
  bind_keys(
    o.list_unhighlight_all_keybind,
    "list-unhighlight-all",
    list_unhighlight_all
  )
  ...
  for i = 1, #o.list_filter_jump_keybind do
    mp.add_forced_key_binding(
      o.list_filter_jump_keybind[i][1],
      "list-filter-jump" .. i,
      function()
...
function update_search_results(character, action)
  ...
  if
    character ~= "" and #list_contents > 0
    or action ~= nil and #list_contents > 0
  then
...
function history_blacklist_check()
  if
    not o.history_blacklist[1]
    or #o.history_blacklist == 1 and o.history_blacklist[1] == ""
  then
    return false
  end
    ...
    then
      ...
      for i = 1, #o.history_blacklist do --1.1.2# loop through blacklisted items, if the blacklist ends with * and it is a match after subbing of the current filePath then log it. #and additionally if it is the exact same path then ignore it.
        if
          ...
          and string.lower(o.history_blacklist[i]:sub(1, -2))
            ~= string.lower(filePath):match("(.*[\\/])")
        then
          msg.info(blacklist_msg)
          return invertable_return[1]
        end
  ...
  elseif starts_protocol(protocols, filePath) then
    ...
    elseif filePath:find("https?://") == 1 then
      ...
      for i = 1, #different_checks do
        if
          different_checks[i]
            and has_value(o.history_blacklist, different_checks[i], nil)
          or different_checks[i] .. "/"
            and has_value(o.history_blacklist, different_checks[i] .. "/", nil)
        then
          msg.info(blacklist_msg)
          return invertable_return[1]
...
function mark_chapter()
  ...
  for i = 1, #list_contents do
    if
      list_contents[i].found_path == filePath
      and tonumber(list_contents[i].found_time) > 0
    then
...
function write_log(target_time, update_seekTime, entry_limit)
  ...
  elseif
    o.file_title_logging == "protocols"
    and (starts_protocol(o.logging_protocols, filePath))
  then
    f:write(
      ('[%s] "%s" | %s | %s | %s'):format(
        os.date(o.date_format),
  ...
  elseif
    o.file_title_logging == "protocols"
    and not (starts_protocol(o.logging_protocols, filePath))
  then
    f:write(
      ("[%s] %s | %s | %s"):format(
        os.date(o.date_format),
...
function history_incognito_mode()
  if not incognito_mode then
    ...
    elseif
      o.restore_incognito_entry == "deleted-restore"
      and autosaved_entry == "autosaved-restore"
    then
      history_fileonly_save()
      autosaved_entry = true
      if list_drawn then
...
function history_resume_option()
  if o.resume_option == "notification" or o.resume_option == "force" then
    ...
    for i = #list_contents, 1, -1 do
      if
        list_contents[i].found_path == video_path
        and tonumber(list_contents[i].found_time) > 0
      then --1.1.4# instead of filePath in case it is causing issue
    ...
    if logged_time > 0 then
      ...
      if o.resume_option == "notification" then
        if
          percentage > o.resume_option_threshold
            and percentage < (100 - o.resume_option_threshold)
...
mp.observe_property("idle-active", "bool", function(_, v)
  ...
  elseif
    v
    and o.startup_idle_behavior == "resume-notime"
    and not loadTriggered
  then --1.1.5# option to load last item on startup
    history_load_last()
  ...
  if
    ...
    or not v
      and o.auto_run_incognito_mode
      and not incognito_auto_run_triggered
  then
