...
local settings = {
  ...
  key_moveup = "UP WHEEL_UP k",
  key_movedown = "DOWN WHEEL_DOWN j",
  key_execute = "ENTER MBTN_LEFT",
  key_closemenu = "ESC MBTN_RIGHT q",
}
...
local file = assert(
  io.open(
    mp.command_native({ "expand-path", "~~/script-opts" }) .. "/menu.json"
  )
)
...
if menu_items == nil then
  error(
    "Invalid JSON format in menu.json. Please run it through a linter. The script is disabled."
  )
end
...
if menu_items and menu_size > 0 then
  ...
  mp.add_key_binding("M", "simplemenu-toggle", toggle_menu)
end
